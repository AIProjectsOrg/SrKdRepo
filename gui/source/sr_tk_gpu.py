...
def pick_providers(mode: str) -> list[str]:
    ...
    if mode == "Auto":
        ...
        elif has_dml() and dml_name:
            order.append(dml_name)
        order.append("CPUExecutionProvider")
    elif mode == "CUDA":
        order = ["CUDAExecutionProvider"] if has_cuda() else ["CPUExecutionProvider"]
    elif mode == "DirectML":
        order = [dml_name, "CPUExecutionProvider"] if (has_dml() and dml_name) else ["CPUExecutionProvider"]
    else:
...
class SRApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("SwinIR / MicroSR • ONNX Runtime (CUDA / DirectML / CPU)")
        ...
        self.providers_used: list[str] | None = None

        # UI variables
        self.provider_mode = tk.StringVar(value="Auto")
        self.window_size_var = tk.IntVar(value=8)
        self.normalize_var = tk.StringVar(value="0to1")
        self.status_var = tk.StringVar(value="Load an ONNX model and an LR image to begin.")
        self.available_eps_var = tk.StringVar(value=str(ort.get_available_providers()))
        self.session_eps_var = tk.StringVar(value="—")
    ...
    def _build_ui(self):
        ...
        ttk.Button(top, text="Load ONNX", command=self.load_model).grid(row=0, column=0, padx=4)
        ttk.Button(top, text="Load LR Image", command=self.load_image).grid(row=0, column=1, padx=4)

        ttk.Label(top, text="Provider:").grid(row=0, column=2, padx=(16, 4))
        self.provider_combo = ttk.Combobox(top, width=10, state="readonly",
                                           values=["Auto", "CUDA", "DirectML", "CPU"],
                                           textvariable=self.provider_mode)
        self.provider_combo.grid(row=0, column=3, padx=4)

        ttk.Label(top, text="Window size:").grid(row=0, column=4, padx=(16, 4))
        self.ws_spin = ttk.Spinbox(top, from_=1, to=64, width=5, textvariable=self.window_size_var)
        self.ws_spin.grid(row=0, column=5, padx=4)

        ttk.Label(top, text="Normalize:").grid(row=0, column=6, padx=(16, 4))
        self.norm_combo = ttk.Combobox(top, width=7, state="readonly",
                                       values=["0to1", "-1to1"],
                                       textvariable=self.normalize_var)
        self.norm_combo.grid(row=0, column=7, padx=4)

        self.run_btn = ttk.Button(top, text="Run Inference", command=self.run_inference, state="disabled")
        self.run_btn.grid(row=0, column=8, padx=(16, 4))

        self.save_btn = ttk.Button(top, text="Save SR", command=self.save_sr, state="disabled")
        self.save_btn.grid(row=0, column=9, padx=4)

        # EP info
        ttk.Label(top, text="Avail EPs:").grid(row=1, column=0, sticky="w", padx=(0, 4))
        ttk.Label(top, textvariable=self.available_eps_var).grid(row=1, column=1, columnspan=5, sticky="w")
        ttk.Label(top, text="Session EPs:").grid(row=1, column=6, sticky="e", padx=(16, 4))
        ttk.Label(top, textvariable=self.session_eps_var).grid(row=1, column=7, columnspan=5, sticky="w")
        ...
        self.status_lbl = ttk.Label(bottom, textvariable=self.status_var, anchor="w")
        self.status_lbl.grid(row=0, column=0, sticky="ew")
        self.pbar = ttk.Progressbar(bottom, mode="indeterminate", length=220)
    ...
    def load_model(self):
        ...
        if not path:
            return
        try:
            providers = pick_providers(self.provider_mode.get())
            self.status_var.set(f"Loading model with providers: {providers}")
            ...
            self.session_eps_var.set(str(self.providers_used))
            self.status_var.set(f"Loaded: {Path(path).name} • IO: {self.io_names[0]} → {self.io_names[1]}")
            self._refresh_buttons()
        except Exception as e:
            self.sess = None
            self.io_names = None
            self.session_eps_var.set("—")
    ...
    def _infer_worker(self):
        try:
            wsize = int(self.window_size_var.get())
            norm = self.normalize_var.get()

            sr_bgr, latency, scale = run_sr_session(self.sess, self.lr_bgr, window_size=wsize, normalize=norm)
            self.after(0, self._on_infer_done, sr_bgr, latency, scale)
        except Exception as e:
            self.after(0, self._on_infer_fail, str(e))

    def _on_infer_done(self, sr_bgr: np.ndarray, latency: float, scale: int):
        self.pbar.stop()
        self.sr_bgr = sr_bgr
        self._draw_sr()
        # Note: Session may offload some ops to CPU; we still show primary EPs
        used = self.sess.get_providers() if self.sess else []
        self.status_var.set(f"Inference OK • ×{scale} • {latency:.2f}s • EPs: {used}")
        self._refresh_buttons()
